...
namespace NetBannerNG.LittleForker
{
    ...
    public sealed class ProcessSupervisor : IDisposable
    {
        ...
        private readonly StateMachine<State, Trigger> _processStateMachine
            = new(State.NotStarted, FiringMode.Immediate);
        ...
        ///     A flag to allow user impersonation in process start.
        /// </param>
            ...
            bool impersonate = false)
        {
            ...
            {
                throw new ArgumentException($"'{nameof(workingDirectory)}' cannot be null or empty.", nameof(workingDirectory));
            }

            if (string.IsNullOrEmpty(processPath))
            {
                throw new ArgumentException($"'{nameof(processPath)}' cannot be null or empty.", nameof(processPath));
            }
        ...
        public async Task Stop(TimeSpan? timeout = null)
        {
            await await _taskQueue
                .Enqueue(() => _processStateMachine.FireAsync(_stopTrigger, timeout).ConfigureAwait(false))
        ...
        private void OnStart()
        {
            ...
            try
            {
                ...
                _process.Exited += (_, _) => _taskQueue.Enqueue(() => _processStateMachine.Fire(Trigger.ProcessExit));
                if (_impersonate)
                {
                    // run impersonated
                }
                ...
                {
                    _ = _process.Start(); // TODO: Add a flag and change this to run as Active user
                }
        ...
        private async Task OnStop(TimeSpan? timeout)
        {
            if (!timeout.HasValue || timeout.Value <= TimeSpan.Zero)
            {
                try
                {
                    if (_process == null)
                    {
                        _logger.LogInformation("No process found to kill.");
                        return;
                    }
            ...
            else
            {
                try
                {
                    ...
                    if (_process == null || ProcessInfo == null)
                    {
                        _logger.LogInformation("No running process found to stop.");
                        return;
                    }
