...
namespace NetBannerNG.LittleForker
{
    ...
    public static class CooperativeShutdown
    {
        ...
        /// </returns>
        public static Task<IDisposable> Listen(Action shutdownRequested, ILoggerFactory loggerFactory, Action<Exception>? onError = null)
        {
            ArgumentNullException.ThrowIfNull(loggerFactory);

            var listener = new CooperativeShutdownListener(
                GetPipeName(Environment.ProcessId),
                shutdownRequested,
        ...
        public static async Task SignalExit(int processId, ILoggerFactory loggerFactory)
        {
            ...
            try
            {
                ...
                using (var streamWriter = new StreamWriter(pipe))
                {
                    using var streamReader = new StreamReader(pipe, true);
        ...
        private sealed class CooperativeShutdownListener : IDisposable
        {
            ...
            internal async Task Listen()
            {
                while (!_stopListening.IsCancellationRequested)
                {
                    ...
                    using var pipe = new NamedPipeServerStream(_pipeName,
                        PipeDirection.InOut,
